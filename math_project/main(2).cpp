 ...
 * 注意事项:
 * 0.bgm那块我没写.
 * 1.由于结果可能是无限循环小数.所以计算结果不要求精确数字,保留小数点后两位即可.
 * 2.由于不保留精确数字,所以不能用==号判断输入与答案是否相等,因为答案是精确的而输入是不精确
 *   的,所以采用判断两数之差的绝对值是否小于一个足够小的数来判断是否相等。
 * 3.这个足够小的数没有精确验证,可以根据自身需求调整(就在main函数里);
 * 4.算式不会生成负数(运算起来有点麻烦,因为负数和减号容易混淆),但是运算过程中可以产生负数,
 *   结果也可能是负数(因为运算起来就不是字符串了)。
 * 5.算式不会生成小括号,放小学生一条生路(括号生成有点麻烦).
 * 6.下面还有一大段注释.
 */
...
/*
    起手式（照抄就行，这玩意我写的垃圾,用起来比较复杂,屎山代码）
    Stack stack;               (创建一个栈);
    char src[1024],dest[1024]; (名字随便,长度随便,保证够用就行);
    memset(src,0,sizeof(src));
    memset(dest,0,sizeof(dest));
    //后面可以自由发挥,我做个示范;

    convert(&stack,src,dest);  (转换为后缀表达式)
    double result = complexAdd(&stack,dest);   (返回值是double类型);
    printf("%f",result);  (打印结果还是留着运算自己发挥就行);
